    ...
    func obtainCustomArguments(completion: @escaping ArgumentsCompletion) {
        ...
        obtainPuzzleSize { [weak self] quit in
            ...
            if quit {
                ...
                self.obtainPassesCount { [weak self] quit in
                    guard let self = self else { return }
                    
                    if quit {
                        completion(nil, nil, nil)
                    } else {
                        self.obtainSolvability { [weak self] quit in
                            guard let self = self else { return }
                            
                            if quit {
                                completion(nil, nil, nil)
                            } else {
                                completion(self.size, self.iterations, self.solvability)
                            }
                        }
                    }
                }
